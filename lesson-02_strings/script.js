...
window.onload = function() {
  ...
  const checkTest = (event) => {
    let str, letterToCount;
    switch (event.target.dataset.field) {
      case 'submitTest1':
        str = prompt('Строка для проверки', 'Quick brown fox jumped Over the lazy dog.');
        letterToCount = prompt('Буква для подсчета', 'o');
        alert('В строке <' + str + '> кол-во букв <' + letterToCount + '>: ' +
          countLetters(str, letterToCount));
        break;
      case 'submitTest2':
        alert('RLE: ' +
          rle(prompt('Строка для проверки RLE', 'AVVVBBBVVXDHJFFFFDDDDDDHAAAAJJJDDSLSSSDDDD')));
        break;
      case 'submitTest3':
        bullsAndCows();
        break;
  ...
  function countLetters(str, letterToCount) {
    const re = RegExp(letterToCount, 'gi');
  ...
  function rle(source) {
    ...
    /*каждую букву строки начиная со второй сравниваем с предыдущей,
  ...
  const convertedString = rle('AVVVBBBVVXDHJFFFFDDDDDDHAAAAJJJDDSLSSSDDDD');
  const expectedString = 'AV3B3V2XDHJF4D6HA4J3D2SLS3D4';
  console.assert(
  ...
  function getCowsCount(numberString, secret) {
    ...
    for (let i = 0; i < numberString.length; i++) {
      const position = secret.indexOf(numberString[i]);
